...
watch(
    ...
    (newVal) =>
        validateTime(
            errors,
            { mins: newVal, sec: time.sec },
            { mins: minsRef, sec: secRef }
        )
);
watch(
    () => time.sec,
    (newVal) =>
        validateTime(
            errors,
            { mins: time.mins, sec: newVal },
            { mins: minsRef, sec: secRef }
        )
);
watch(
    () => tempVal.value,
    (newVal) => validateTemp(newVal, errors, tempRef)
);
watch(
    () => formData.temperature.sensor,
...
<template>
    <div class="min-h-screen bg-white p-4 md:p-8">
        <h1 class="bg-main text-lg font-bold text-white rounded-lg p-4 mb-6 shadow max-w-screen-md mx-auto">
            Record Measurement
        </h1>
        <div class="bg-light rounded-lg p-4 mb-6 shadow max-w-screen-md mx-auto">
            ...
            <div class="mb-4">
                ...
                <input ref="loc" type="text" id="location"
                    class="w-full border border-gray-300 rounded px-3 py-2 mb-3 focus:outline-none focus:ring-2 focus:ring-primary" />
            </div>
            <div class="flex items-center gap-4 mb-3">
                <button type="button" @click="getLocation" :disabled="locating"
                    class="bg-main mb-4 text-white px-4 py-2 rounded disabled:bg-gray-400 disabled:cursor-not-allowed">
                    {{ locating ? "Locating…" : "Use Current Location" }}
                </button>
                <p v-if="!locAvail">
                    Location not available, please select your location manually.
                </p>
            </div>
            ...
            <div class="flex-start min-w-0 flex items-center gap-2">
                ...
                <select id="water_source" v-model="formData.waterSource"
        ...
        <div v-if="selectedMetrics.includes('temperature')"
            ...
            <div class="flex-1 items-start gap-4 mb-4">
                <div class="flex flex-col">
                    <div class="flex-start min-w-0 flex items-center gap-2">
                        ...
                        <input id="sensor-type" v-model="formData.temperature.sensor" placeholder="thermometer"
                            type="text" :class="[
                ...
                <div class="flex-1 flex-col">
                    <div class="flex items-center gap-4">
                        <div class="flex-1 min-w-0 flex items-center gap-2">
                            ...
                            <input id="temp-val" v-model="tempVal" type="number" @input="onTempInput" ref="tempRef"
                                placeholder="e.g. 24.3" :class="[
            ...
            <div class="flex items-center gap-2">
                ...
                <div class="flex flex-col">
                    <div class="flex items-center gap-2">
                        <input id="time-waited_min" @input="onMinsInput" v-model="time.mins" placeholder="00"
                            type="number" ref="minsRef" :class="[
                ...
                <div class="flex flex-col">
                    <div class="flex items-center gap-2">
                        <input id="time-waited_sec" @input="onSecInput" v-model="time.sec" placeholder="00"
                            type="number" ref="secRef" :class="[
                                'w-16 border border-gray-300 rounded px-2 py-1',
        ...
        <!-- Submit -->
        <div class="flex grow justify-evenly items-center mt-6 gap-2">
            <button type="button"
                class="flex-grow bg-white border border-primary text-primary px-4 py-2 rounded hover:bg-primary-light"
                @click="clear">
                Clear
            </button>
            <button :disabled="!validated" type="submit" @click="postData" style="background-color: #00a6d6"
                class="flex-grow px-4 py-2 rounded text-white disabled:bg-gray-400 disabled:opacity-50 cursor-not-allowed">
                Submit
            </button>
        </div>
    </div>
